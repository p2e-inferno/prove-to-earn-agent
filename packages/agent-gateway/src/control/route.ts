...
export function createHeadlessControlRoute(options: {
  ...
  return async (
    ...
    if (!headlessAgentApiEnabled())
      return controlJson({ ok: false, code: "NOT_FOUND" }, 404);
    const actor = await resolveHeadlessActor(req);
    if (!actor) {
      const response = controlJson(
        { ok: false, code: "INVALID_TOKEN", retryable: false },
        401,
      );
    ...
    if (!actor.claims.scopes.includes(options.scope)) {
      return controlJson(
        { ok: false, code: "INSUFFICIENT_SCOPE", retryable: false },
        403,
      );
    }
    ...
    const limit = await rateLimiter.check(
      `headless-api:${clientHash}`,
      120,
      60_000,
    );
    if (limit.unavailable)
      return controlJson(
        { ok: false, code: "RATE_LIMIT_UNAVAILABLE", retryable: true },
        503,
      );
    if (!limit.success)
      return controlJson(
    ...
    if (options.mutation) {
      ...
      if (!requestId || requestId.length > 200) {
        return controlJson(
          { ok: false, code: "IDEMPOTENCY_KEY_REQUIRED", retryable: false },
          400,
        );
      }
    }
    ...
    const runInner = async (): Promise<NextResponse> => {
      try {
          ...
          code === "PAID_ACCESS_REQUIRED" || code === "POLICY_DENIED"
            ...
            : code.includes("NOT_FOUND")
              ...
              : code.includes("CONFLICT") ||
                ...
                : 503;
        return controlJson(
          { ok: false, code, retryable: status >= 500 },
          status,
        );
      }
    };

    if (!options.economicRoute) return runInner();
    return invokeCanonicalEconomicRoute(req, {
      routeId: options.economicRoute.id,
      params: options.economicRoute.params?.(params),
      handler: runInner,
    });
  };
}
