...
export function classifyFailure(code: string | undefined): FailureClass {
  ...
  if (
    /X402_(?:PAYMENT_)?VALIDATION_FAILED|X402_(?:CONFIGURATION|PAYER)_MISMATCH|UNSUPPORTED|INVALID|NOT_FOUND|ALREADY_SELECTED|NOTHING_EXECUTABLE|REVOKED|EXPIRED|FORBIDDEN/.test(
      normalized,
    )
  ) {
    return "fatal";
